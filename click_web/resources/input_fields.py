...
class FolderInput(BaseInput):
    def is_supported(self):
        if isinstance(self.param.type, click.Path) and self.param.type.dir_okay:
            return True
        return False
...
class FileInput(BaseInput):
    def is_supported(self):
        if isinstance(self.param.type, click.File) or isinstance(self.param.type, click.Path) and self.param.type.file_okay:
            return True
        return False
...
"""
INPUT_TYPES = [ChoiceInput, FlagInput, IntInput, FloatInput, FolderInput, FileInput, EmailInput, PasswordInput, TextAreaInput]
