...
class Executor:
    ...
    def _run_script_and_generate_stream(self):
        ...
        process = subprocess.Popen(
            self._command_line.get_commandline(), shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    def _create_result_footer(self):
        ...
        if self.returncode == 0:
            ...
            lines.append(
                f'<div class="script-exit script-exit-error">' f"Script exited with error code: {self.returncode}</div>"
            )
...
class FormToCommandLineBuilder:
    ...
    def _is_option(cmd_option):
        return isinstance(cmd_option, str) and (cmd_option.startswith(("--", "-")))
...
class FieldInfo:
    ...
    def factory(key):
        ...
        if is_file:
            field_info = FieldFileInfo(field_id) if is_uploaded else FieldOutFileInfo(field_id)
        elif is_path:
            field_info = FieldPathInfo(field_id) if is_uploaded else FieldPathOutInfo(field_id)
        else:
...
class FieldFileInfo(FieldInfo):
    ...
    def __init__(self, fimeta):
        ...
        self.generate_download_link = "w" in self.mode
